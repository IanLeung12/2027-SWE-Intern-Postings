...
function excluded(company,blocked){var c=norm(company),a=acronym(company);
 return blocked.some(function(x){var b=norm(x);return b&&(c===b||c.indexOf(b)>-1||
  (b.length>=3&&a===b));});}
function apply(){var t=q.value.toLowerCase(),blocked=list();
  ...
  var okT=!t||txt.indexOf(t)>-1,okE=!excluded(company,blocked),okF;
  if(f==='all')okF=true;
